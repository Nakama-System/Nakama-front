...
// CONSTANTES
// ═══════════════════════════════════════════════════════════
const API_BASE = "https://nakama-backend-render.onrender.com";
...
// ═══════════════════════════════════════════════════════════
// generateQuestions — MISMO NOMBRE, ahora consulta el backend
// Es async: devuelve Promise<Question[]>
// ═══════════════════════════════════════════════════════════
async function generateQuestions(categorias: string[], count = MAX_PREGUNTAS): Promise<Question[]> {
  ...
  try {
    ...
    // Mismo comportamiento que antes: filtrar por categoría, si no hay suficientes usar todo el pool
...
export default function BattleGame({
  ...
  // ✅ preguntas ahora se cargan async al montar
  ...
  useEffect(() => {
    setLoadingQ(true);
    generateQuestions(categorias).then(qs => {
      setQuestions(qs);
      setLoadingQ(false);
    });
  ...
  // ── Countdown KOF ──────────────────────────────────────
  useEffect(() => {
    // No arrancar countdown hasta que las preguntas estén listas
    if (loadingQ || phase !== "countdown") return;
    if (countdown === 0) {
      setShowGo(true);
  ...
  const commitAnswer = useCallback((opt: number | null) => {
    ...
    const correcto = opt !== null && currentQ && opt === currentQ.correcta;
    const pts      = correcto ? (pwr === "D" ? PTS_PIEDAD : PTS_CORRECTA) : 0;
    const myAns: PlayerAnswer = { userId: myUserId, opcion: opt, tiempo: myTime, correcto: !!correcto, pts, poder: pwr ?? undefined };
  ...
  const handleFight = () => {
    // ✅ Revancha: cargar preguntas nuevas del backend
    setLoadingQ(true);
    generateQuestions(categorias).then(qs => {
      setQuestions(qs);
      setLoadingQ(false);
  ...
  // ✅ Pantalla de carga mientras se obtienen las preguntas
  if (loadingQ) return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      ...
      <Spinner size={44}/>
      <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"0.82rem", fontFamily:"'Segoe UI',sans-serif", margin:0 }}>
        Cargando preguntas...
      </p>
    </div>
  );
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:999, background:"#000", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", overflow:"hidden" }}>
      ...
      {["top-left","top-right","bottom-left","bottom-right"].map(pos => (
        <div key={pos} style={{ position:"absolute", top:pos.includes("top")?20:undefined, bottom:pos.includes("bottom")?20:undefined, left:pos.includes("left")?20:undefined, right:pos.includes("right")?20:undefined, width:48, height:48, border:`3px solid ${color}`, borderRight:pos.includes("right")?`3px solid ${color}`:"none", borderLeft:pos.includes("left")?`3px solid ${color}`:"none", borderTop:pos.includes("top")?`3px solid ${color}`:"none", borderBottom:pos.includes("bottom")?`3px solid ${color}`:"none", opacity:0.6, transition:"all 0.3s" }}/>
      ))}
...
// QUESTION SCREEN
// ═══════════════════════════════════════════════════════════
function QuestionScreen({ question, qIndex, total, timeLeft, players, myUserId, myAnswer, roundAnswers, eliminated, powers, activePower, hiddenOpts, phase, onAnswer, onPower, onNext }: {
  question: Question; qIndex: number; total: number; timeLeft: number; players: BattlePlayer[]; myUserId: string; myAnswer: number | null; roundAnswers: PlayerAnswer[]; eliminated: Set<string>; powers: PowerState; activePower: string | null; hiddenOpts: number[]; phase: GamePhase; onAnswer: (opt: number) => void; onPower: (key: "A"|"B"|"C"|"D") => void; onNext: () => void;
}) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", fontFamily:"'Arial Black', Impact, sans-serif", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 16px", background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, color: danger ? "#ef4444" : "#fff", animation: danger ? "dangerPulse 0.5s ease infinite" : "none" }}>
          <span style={{ fontSize:"1.4rem", fontWeight:900 }}>{timeLeft}</span>
          <span style={{ fontSize:"0.6rem", opacity:0.5 }}>SEG</span>
        </div>
      </div>
      <div style={{ height:4, background:"rgba(255,255,255,0.06)" }}>
        <div style={{ height:"100%", width:`${pct}%`, background: danger ? "linear-gradient(90deg,#ef4444,#ff0000)" : "linear-gradient(90deg,#f97316,#fbbf24)", boxShadow: danger ? "0 0 12px #ef4444" : "0 0 12px #f97316", transition:"width 1s linear" }}/>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:`repeat(${Math.min(players.filter(p=>!eliminated.has(p.userId)).length,4)}, 1fr)`, gap:8, padding:"10px 12px" }}>
        {players.filter(p => !eliminated.has(p.userId)).map(p => {
          ...
          return (
            <div key={p.userId} style={{ background: isMe ? "linear-gradient(135deg,rgba(249,115,22,0.12),rgba(249,115,22,0.06))" : "rgba(255,255,255,0.03)", border:`1.5px solid ${isMe ? "#f97316" : "rgba(255,255,255,0.07)"}`, borderRadius:10, padding:"10px 8px", display:"flex", flexDirection:"column", alignItems:"center", gap:6, transition:"all 0.3s" }}>
              <div style={{ position:"relative" }}>
                ...
                {isResult && hasAns && (
                  <div style={{ position:"absolute", bottom:-2, right:-2, width:16, height:16, borderRadius:"50%", background: ans.correcto ? "#22c55e" : "#ef4444", border:"2px solid #080810", display:"flex",alignItems:"center",justifyContent:"center", fontSize:"0.55rem" }}>
                    {ans.correcto ? "✓" : "✗"}
                  </div>
                )}
                {!isResult && (
                  <div style={{ position:"absolute", bottom:-2, right:-2, width:14, height:14, borderRadius:"50%", background: (isMe && myAnswer !== null) ? "#22c55e" : (hasAns ? "#22c55e" : "#6b7280"), border:"2px solid #080810", animation: (!isMe && !hasAns && !isResult) ? "waitPulse 1.5s ease infinite" : "none" }}/>
                )}
              </div>
              ...
              {isResult && hasAns && (
                <div style={{ fontSize:"0.6rem", color: ans.correcto ? "#22c55e" : "#ef4444", fontFamily:"'Segoe UI',sans-serif", textAlign:"center" }}>
                  {ans.correcto ? `+${ans.pts} pts` : "Sin puntos"}
                  {ans.poder === "D" && ans.correcto && <span style={{color:"#f59e0b"}}> (Piedad)</span>}
                  <br/><span style={{color:"rgba(255,255,255,0.3)"}}>en {ans.tiempo}s</span>
                </div>
              )}
            </div>
          );
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"8px 16px 0" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, flex:1 }}>
          {question.opciones.map((opt, i) => {
            ...
            let bg = "rgba(255,255,255,0.04)", border = "1px solid rgba(255,255,255,0.1)", color = "#fff";
            ...
            return (
              <button key={i} disabled={hidden || isResult || myAnswer !== null} onClick={() => onAnswer(i)} style={{ background:bg, border, borderRadius:10, color, padding:"12px 14px", cursor:(!hidden && !isResult && myAnswer===null)?"pointer":"default", textAlign:"left", display:"flex", alignItems:"center", gap:10, transition:"all 0.2s", fontFamily:"'Segoe UI',sans-serif", transform: selected ? "scale(1.01)" : "scale(1)" }}>
                ...
                <span style={{fontSize:"0.82rem", fontWeight:600, lineHeight:1.3}}>{hidden && !isResult ? "••••" : opt}</span>
        ...
        <div style={{ display:"flex", gap:6, padding:"10px 0 6px", justifyContent:"center" }}>
          {PODERES_INFO.map(p => {
            ...
            return (
              <button key={p.key} onClick={() => onPower(p.key)} disabled={!avail} title={`${p.nombre}: ${p.desc}`} style={{ width:48, height:48, borderRadius:10, background: used ? "rgba(255,255,255,0.02)" : avail ? `rgba(${hexToRgb(p.color)},0.12)` : "rgba(255,255,255,0.02)", border:`1.5px solid ${used ? "rgba(255,255,255,0.05)" : avail ? p.color : "rgba(255,255,255,0.05)"}`, cursor: avail ? "pointer" : "not-allowed", display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center", gap:2, opacity: used ? 0.25 : 1, transition:"all 0.2s" }}>
...
// ROUND RESULT SCREEN
// ═══════════════════════════════════════════════════════════
function RoundResultScreen({ players, eliminated, onContinue }: {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center", padding:24, gap:24, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10, width:"100%", maxWidth:360 }}>
        {active.map((p,i) => (
          <div key={p.userId} style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderRadius:12, background:`linear-gradient(135deg,rgba(249,115,22,0.08),rgba(249,115,22,0.03))`, border:`1.5px solid ${i===0?"#f97316":"rgba(255,255,255,0.08)"}` }}>
      ...
      {elim.length > 0 && (
        <>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, width:"100%", maxWidth:360 }}>
            {elim.map(p => (
              <div key={p.userId} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", borderRadius:10, background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.15)", opacity:0.7 }}>
      ...
      <button onClick={onContinue} style={{ marginTop:8, background:"linear-gradient(135deg,#f97316,#e05000)", border:"none", color:"#fff", fontWeight:900, padding:"14px 48px", borderRadius:12, cursor:"pointer", fontSize:"1rem", letterSpacing:"0.08em", boxShadow:"0 4px 24px rgba(249,115,22,0.4)" }}>
        CONTINUAR ⚔️
      </button>
    </div>
  );
...
// DUEL SCREEN
// ═══════════════════════════════════════════════════════════
function DuelScreen({ players, onPact, onFight }: {
  players: BattlePlayer[]; onPact: () => void; onFight: () => void;
}) {
  const [p1, p2] = players;
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center", padding:24, gap:20, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
      <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at center, rgba(249,115,22,0.06) 0%, transparent 70%)", pointerEvents:"none" }}/>
      <div style={{ fontSize:"0.65rem", letterSpacing:"0.4em", color:"#f97316" }}>¡EMPATE!</div>
      <h2 style={{ color:"#fff", margin:0, fontSize:"clamp(1.6rem,5vw,2.4rem)", textAlign:"center" }}>DUELO FINAL</h2>
      ...
      <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"0.8rem", fontFamily:"'Segoe UI',sans-serif", textAlign:"center", maxWidth:280, lineHeight:1.6 }}>
        Ambos tienen el mismo puntaje. Pueden pelear en un duelo con preguntas nuevas, o hacer un pacto de caballeros y repartirse los premios.
      </p>
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center" }}>
        <button onClick={onFight} style={{ background:"linear-gradient(135deg,#ef4444,#b91c1c)", border:"none", color:"#fff", fontWeight:900, padding:"14px 32px", borderRadius:12, cursor:"pointer", fontSize:"0.9rem", letterSpacing:"0.06em", boxShadow:"0 4px 20px rgba(239,68,68,0.35)" }}>⚔️ PELEAR</button>
        <button onClick={onPact} style={{ background:"rgba(255,255,255,0.04)", border:"1.5px solid rgba(255,255,255,0.15)", color:"rgba(255,255,255,0.8)", fontWeight:900, padding:"14px 32px", borderRadius:12, cursor:"pointer", fontSize:"0.9rem", letterSpacing:"0.06em" }}>🤝 PACTO DE CABALLEROS</button>
      </div>
    </div>
  );
}
...
function PactScreen({ players, onFinalize }: { players: BattlePlayer[]; onFinalize: () => void }) {
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center", padding:24, gap:20, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      <span style={{fontSize:"3rem"}}>🤝</span>
      <h2 style={{color:"#fff",margin:0,fontSize:"clamp(1.6rem,5vw,2.2rem)",textAlign:"center"}}>PACTO DE CABALLEROS</h2>
      <p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.85rem",fontFamily:"'Segoe UI',sans-serif",textAlign:"center",maxWidth:300}}>La sabiduría y el respeto mutuo llevan a un acuerdo honorable.</p>
      <div style={{display:"flex",gap:20,alignItems:"center"}}>
        {players.map(p => (
          <div key={p.userId} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:10, padding:"20px 24px", borderRadius:14, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)" }}>
            ...
            <div style={{color:"#f59e0b",fontSize:"1.4rem",fontWeight:900}}>{PRIZE_PACT.toLocaleString()}<span style={{fontSize:"0.6rem",color:"#6b7280",marginLeft:4}}>pts</span></div>
          </div>
        ))}
      </div>
      <p style={{color:"rgba(255,255,255,0.3)",fontSize:"0.72rem",fontFamily:"'Segoe UI',sans-serif"}}>Cada uno recibe {PRIZE_PACT} puntos</p>
      <button onClick={onFinalize} style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)", border:"none", color:"#000", fontWeight:900, padding:"14px 40px", borderRadius:12, cursor:"pointer", fontSize:"0.95rem", letterSpacing:"0.06em", boxShadow:"0 4px 20px rgba(245,158,11,0.4)" }}>
...
// FINAL RESULT SCREEN
// ═══════════════════════════════════════════════════════════
function FinalResultScreen({ players, winnerId, myUserId, onFinalize }: {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex",flexDirection:"column",alignItems:"center", padding:"24px 16px 32px", gap:20, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
          ...
          return (
            <div key={p.userId} style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderRadius:12, position:"relative", overflow:"hidden", border:`${isWin?1.5:1}px solid ${isWin?"rgba(251,191,36,0.5)":"rgba(255,255,255,0.06)"}` }}>
              ...
              <div style={{ flex:1, zIndex:1 }}>
                ...
                <div style={{ fontSize:"0.65rem", color: isWin ? "rgba(251,191,36,0.6)" : "rgba(255,255,255,0.15)", fontFamily:"'Segoe UI',sans-serif" }}>{isWin ? `Gana ${prize.toLocaleString()} pts` : `Obtiene ${p.score.toLocaleString()} pts`}</div>
              </div>
      ...
      <button onClick={onFinalize} style={{ background:"linear-gradient(135deg,#f97316,#e05000)", border:"none", color:"#fff", fontWeight:900, padding:"14px 48px", borderRadius:12, cursor:"pointer", fontSize:"0.95rem", letterSpacing:"0.08em", boxShadow:"0 4px 24px rgba(249,115,22,0.4)", zIndex:1, marginTop:8 }}>
        FINALIZAR BATALLA
      </button>
...
function AvatarSmall({ url, name }: { url?: string | null; name: string }) {
  return (
    <div style={{ width:32, height:32, borderRadius:"50%", overflow:"hidden", flexShrink:0, border:"2px solid rgba(255,255,255,0.1)" }}>
      {url ? <img src={url} alt={name} style={{width:"100%",height:"100%",objectFit:"cover"}}/> : <div style={{ width:"100%",height:"100%", background:"linear-gradient(135deg,#f97316,#e05000)", display:"flex",alignItems:"center",justifyContent:"center", fontSize:"0.75rem",fontWeight:900,color:"#fff" }}>{name[0].toUpperCase()}</div>}
    </div>
  );
}
