...
// ═══════════════════════════════════════════════════════════
// FALLBACK — se usa si el backend no responde
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// generateQuestions — MISMO NOMBRE, ahora async desde backend
// ═══════════════════════════════════════════════════════════
async function generateQuestions(categorias: string[], count = MAX_PREGUNTAS, token?: string | null): Promise<Question[]> {
  const safeCats = Array.isArray(categorias) ? categorias : [];

  try {
    const t = token ?? (typeof window !== "undefined" ? localStorage.getItem("nakama_token") : null);
    const params = new URLSearchParams({ count: String(count) });
    if (safeCats.length > 0) params.set("categorias", safeCats.join(","));
    ...
    if (questions.length === 0) throw new Error("Sin preguntas en servidor");

    return [...questions].sort(() => Math.random() - 0.5).slice(0, count);
  } catch (err) {
    console.warn("[generateQuestions] Fallback a preguntas locales:", err);
    const filtered = FALLBACK_POOL.filter(q => safeCats.includes(q.categoria));
    const base     = filtered.length >= count ? filtered : FALLBACK_POOL;
    return [...base].sort(() => Math.random() - 0.5).slice(0, count);
  }
}
...
export default function BattleGame({
  ...
  const clearRivalTimeout = useCallback(() => {
    if (rivalTimeoutRef.current) { clearInterval(rivalTimeoutRef.current); rivalTimeoutRef.current = null; }
  ...
  const resolveResult = useCallback((currentPlayers: BattlePlayer[]) => {
    ...
    if (isTie) {
      ...
      setDuelPair(pair);
      if (duelRoundRef.current >= 1) { setPhase("pact"); phaseRef.current = "pact"; return; }
  ...
  const startRivalTimeout = useCallback(() => {
    ...
    rivalTimeoutRef.current = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        setRivalCountdown(null);
        if (phaseRef.current === "waiting_game_over") resolveResult(playersRef.current);
      }
    }, 1000);
  }, [resolveResult]);
  ...
  // ═══════════════════════════════════════════════════════
  // generateQuestions — llama al backend, misma firma
  // ═══════════════════════════════════════════════════════
  const loadQuestions = useCallback(async (isRematch = false, socket?: Socket) => {
    setLoadingQ(true);
    try {
      const qs = await generateQuestions(categoriasRef.current, MAX_PREGUNTAS, tokenRef.current);
      questionsRef.current = qs;
      setQuestions(qs);
      const s = socket ?? socketRef.current;
      if (s) {
        const event = isRematch ? "battle:set_questions_rematch" : "battle:set_questions";
        s.emit(event, { roomId, questions: qs });
      }
      return qs;
    } finally {
      setLoadingQ(false);
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      ...
      if (isCreatorRef.current && !creatorSetupDone.current) {
        ...
        // Auto-inicio countdown
        setAutoSeg(AUTO_START_SEG);
        if (autoTimerRef.current) clearInterval(autoTimerRef.current);
        autoTimerRef.current = setInterval(() => {
          setAutoSeg(prev => {
            if (prev === null || prev <= 1) { clearInterval(autoTimerRef.current!); autoTimerRef.current = null; return null; }
            return prev - 1;
          });
        }, 1000);

        // ✅ Cargar preguntas del backend (misma función generateQuestions, ahora async)
        loadQuestions(false, s);
      }
    };
    ...
    const onGameOver = (data: { players: BattlePlayer[]; winnerId: string | null }) => {
      ...
      const duelPhases: GamePhase[] = ["final_result","duel_creator","duel_waiting_decision","duel_waiting_response","duel_response","duel_rejected_result","pact"];
    ...
    s.on("battle:player_done_ack", ({ doneCount, totalCount, pendingUsernames, doneUserIds }: {
      ...
      if (doneSoFar >= activeTotalRef.current && phaseRef.current === "waiting_game_over") {
        setTimeout(() => { if (phaseRef.current !== "waiting_game_over") return; resolveResult(playersRef.current); }, 1000);
      }
    });
    ...
    s.on("battle:player_left", ({ userId }: { userId: string }) => {
      setPlayers(prev => {
        ...
        const doneSoFar = donePlayersRef.current.size;
        if (doneSoFar >= activeTotalRef.current && phaseRef.current === "waiting_game_over") {
          setTimeout(() => { if (phaseRef.current !== "waiting_game_over") return; resolveResult(updated.filter(p => !p.eliminated)); }, 800);
        }
        return updated;
      });
    });
    ...
    return () => {
      s.off("connect", onConnect); s.off("battle:game_questions"); s.off("battle:rematch_questions");
      s.off("battle:game_start"); s.off("battle:player_answered"); s.off("battle:game_over", onGameOver);
      s.off("battle:player_done_ack"); s.off("battle:player_left"); s.off("battle:duel_fight_proposed");
      s.off("battle:duel_pact_proposed"); s.off("battle:duel_accepted"); s.off("battle:duel_rejected");
      s.off("connect_error");
      if (autoTimerRef.current) clearInterval(autoTimerRef.current);
  ...
  useEffect(() => {
    ...
    if (countdown === 0) {
      setShowGo(true);
      setTimeout(() => {
        stopCountdownAudio(); setShowGo(false);
        setQIndex(0);         qIndexRef.current   = 0;
        setMyAnswer(null);    setMyLastResult(null);
  ...
  // ── Rematch — usa generateQuestions (async) ───────────────
  const doRematch = useCallback(async () => {
    ...
    setHidden([]);        setPendingPlayers([]);
    clearRivalTimeout();

    // ✅ Usar generateQuestions (mismo nombre, ahora async)
    await loadQuestions(true, socketRef.current ?? undefined);

    setCd(3);
    setPhase("countdown"); phaseRef.current = "countdown";
  }, [clearRivalTimeout, loadQuestions]);
  ...
  const sendAnswer = useCallback((opt: number | null) => {
    ...
    setMyAnswer(opt); setMyScore(newScore);
    myScoreRef.current = newScore; myFinalScoreRef.current = newScore;
    ...
    socketRef.current?.emit("battle:answer", { roomId, qIndex: qIndexRef.current, userId: myUserId, opcion: opt, tiempo: myTime, correcto, pts });
    setActivePower(null); activePowerRef.current = null;
    setHidden([]);
  ...
  const nextQuestion = useCallback(() => {
    ...
    if (nextIdx >= MAX_PREGUNTAS) {
      ...
      socketRef.current?.emit("battle:player_done", { roomId, userId: myUserId, activeCount: activeTotalRef.current });
      setPendingPlayers([]);
      ...
      if (donePlayersRef.current.size >= activeTotalRef.current) {
        ...
        setTimeout(() => {
          ...
          const currentPlayers = [...playersRef.current].map(p => p.userId === myUserId ? { ...p, score: myScoreAtDone.current } : p);
  ...
  const handleProposePact = useCallback(() => {
    ...
    const t = setTimeout(() => { if (phaseRef.current === "duel_creator") { setPhase("pact"); phaseRef.current = "pact"; } }, 1500);
    socketRef.current?.once("battle:duel_pact_proposed", () => clearTimeout(t));
  }, [roomId]);

  const handleAcceptFight = useCallback(() => { socketRef.current?.emit("battle:duel_accept", { roomId }); }, [roomId]);
  const handleRejectFight = useCallback(() => { socketRef.current?.emit("battle:duel_reject", { roomId, loserId: myUserId }); }, [roomId, myUserId]);
  ...
  // ✅ Pantalla de carga mientras se obtienen las preguntas del backend
  if (phase === "waiting_start" && loadingQ) return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      <div style={{ fontSize:"0.65rem", letterSpacing:"0.4em", color:"#f97316" }}>NAKAMA BATTLE</div>
      <Spinner size={44}/>
      <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"0.82rem", fontFamily:"'Segoe UI',sans-serif", margin:0 }}>Cargando preguntas...</p>
    </div>
  );
  ...
  if (phase === "waiting_game_over") return (
    <WaitingGameOverScreen players={visiblePlayers} myUserId={myUserId} pendingPlayers={pendingPlayers} rivalCountdown={rivalCountdown}/>
  );

  if (phase === "duel_creator") return (
    <DuelCreatorScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))} onFight={handleProposeFight} onPact={handleProposePact}/>
  );
  if (phase === "duel_waiting_decision") return (
    <DuelWaitingDecisionScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))}/>
  );
  if (phase === "duel_waiting_response") return (
    <DuelWaitingResponseScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))}/>
  );
  if (phase === "duel_response") return (
    <DuelResponseScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))} onAccept={handleAcceptFight} onReject={handleRejectFight}/>
  );
  if (phase === "duel_rejected_result") return (
    <DuelRejectedResultScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))} winnerId={duelWinnerId} myUserId={myUserId} onFinalize={handleFinalize}/>
  );
  if (phase === "pact") return (
    <PactScreen players={visiblePlayers.filter(p => duelPair.includes(p.userId))} onFinalize={handleFinalize}/>
  );
  ...
  return <div style={{ minHeight:"100vh", background:"#080810", display:"flex", alignItems:"center", justifyContent:"center" }}><Spinner size={40}/></div>;
}
...
function WaitingStartScreen({ players, isCreator, myUserId, autoSeg, onStart }: {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28, padding:24, fontFamily:"'Arial Black', Impact, sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <h2 style={{ color:"#fff", margin:0, fontSize:"clamp(1.6rem,5vw,2.4rem)", textAlign:"center", zIndex:1 }}>{isCreator ? "¡Todo listo!" : "Esperando al creador..."}</h2>
...
function CountdownScreen({ countdown, showGo }: { countdown: number; showGo: boolean }) {
  ...
  const cornerStyle = (pos: string): React.CSSProperties => ({
    position:"absolute", top:pos.includes("top")?20:undefined, bottom:pos.includes("bottom")?20:undefined,
    left:pos.includes("left")?20:undefined, right:pos.includes("right")?20:undefined,
    width:48, height:48, borderTopWidth:pos.includes("top")?3:0, borderBottomWidth:pos.includes("bottom")?3:0,
    borderLeftWidth:pos.includes("left")?3:0, borderRightWidth:pos.includes("right")?3:0,
...
function WaitingGameOverScreen({ players, myUserId, pendingPlayers, rivalCountdown }: {
  players: BattlePlayer[]; myUserId: string; pendingPlayers: string[]; rivalCountdown: number | null;
}) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, padding:24, fontFamily:"'Arial Black', Impact, sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ zIndex:1 }}><div style={{ fontSize:"3.5rem", animation:"iconBounce 1.6s ease infinite" }}>⏳</div></div>
      <h2 style={{ color:"#fff", margin:0, fontSize:"clamp(1.4rem,4vw,2rem)", textAlign:"center", zIndex:1 }}>¡Terminaste! Esperando al rival...</h2>
      {rivalCountdown !== null && (
        <div style={{ zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:8 }}>
          <div style={{ fontSize:"0.68rem", color:"rgba(255,255,255,0.3)", fontFamily:"'Segoe UI',sans-serif" }}>{rivalCountdown>0?"Resultado automático en":"Calculando resultado..."}</div>
          <div style={{ fontSize:"2.2rem", fontWeight:900, lineHeight:1, color: rivalCountdown<=10?"#ef4444":"#f59e0b", textShadow:`0 0 20px ${rivalCountdown<=10?"rgba(239,68,68,0.6)":"rgba(245,158,11,0.6)"}` }}>{rivalCountdown}s</div>
          ...
          <div style={{ fontSize:"0.65rem", color:"rgba(255,255,255,0.2)", fontFamily:"'Segoe UI',sans-serif" }}>Si el rival no termina, se cuenta con los puntos que lleva</div>
        </div>
      )}
      {me && (
        <div style={{ zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:8, padding:"20px 32px", borderRadius:16, background:"rgba(0,200,255,0.06)", border:"1.5px solid rgba(0,200,255,0.2)" }}>
          ...
          <div style={{ fontSize:"2.2rem", fontWeight:900, color:"#00c8ff", letterSpacing:"-0.02em" }}>{me.score.toLocaleString()}<span style={{ fontSize:"0.7rem", color:"#6b7280", marginLeft:6 }}>pts</span></div>
          ...
          <div key={rival.userId} style={{ zIndex:1, display:"flex", alignItems:"center", gap:12, padding:"14px 20px", borderRadius:12, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", fontFamily:"'Segoe UI',sans-serif", width:"100%", maxWidth:360 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:4 }}>
                {isPending ? (<><span style={{ display:"inline-block", width:8, height:8, borderRadius:"50%", background:"#f59e0b", animation:"waitPulse 1.4s ease infinite" }}/><span style={{ fontSize:"0.72rem", color:"#f59e0b" }}>Respondiendo...</span></>) : (<><span style={{ display:"inline-block", width:8, height:8, borderRadius:"50%", background:"#22c55e" }}/><span style={{ fontSize:"0.72rem", color:"#22c55e" }}>¡Terminó!</span></>)}
              </div>
            </div>
          </div>
        );
      })}
      <style>{`@keyframes waitPulse{0%,100%{opacity:1}50%{opacity:0.25}} @keyframes iconBounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}`}</style>
    </div>
  );
}

// ── Duel screens (DuelCreatorScreen, DuelWaitingDecisionScreen, DuelWaitingResponseScreen,
//    DuelResponseScreen, DuelRejectedResultScreen, PactScreen, FinalResultScreen)
// ── Son idénticos al BattleGame anterior — se mantienen sin cambios ──────────────────────

function DuelCreatorScreen({ players, onFight, onPact }: { players: BattlePlayer[]; onFight: () => void; onPact: () => void }) {
  const [p1, p2] = players;
  return (
...
function DuelWaitingDecisionScreen({ players }: { players: BattlePlayer[] }) {
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, padding:24, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      <div style={{ fontSize:"3rem" }}>⚔️</div>
      <h2 style={{ color:"#fff", margin:0, fontSize:"clamp(1.4rem,4vw,2rem)", textAlign:"center" }}>Esperando la decisión del creador...</h2>
...
function DuelWaitingResponseScreen({ players }: { players: BattlePlayer[] }) {
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, padding:24, fontFamily:"'Arial Black',Impact,sans-serif" }}>
      ...
      <h2 style={{ color:"#fff", margin:0, fontSize:"clamp(1.4rem,4vw,2rem)", textAlign:"center" }}>Esperando respuesta del rival...</h2>
      <div style={{ display:"flex", gap:16 }}>
        {players.map(p => (
          <div key={p.userId} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8, padding:"14px 18px", borderRadius:12, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", fontFamily:"'Segoe UI',sans-serif" }}>
            <AvatarMed url={p.avatarUrl} name={p.username} size={44}/>
...
function DuelResponseScreen({ players, onAccept, onReject }: { players: BattlePlayer[]; onAccept: () => void; onReject: () => void }) {
  const [p1, p2] = players;
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, gap:24, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", gap:14, zIndex:1 }}>
        <button onClick={onReject} style={{ background:"rgba(239,68,68,0.08)", border:"1.5px solid rgba(239,68,68,0.3)", color:"#ef4444", fontWeight:900, padding:"14px 28px", borderRadius:12, cursor:"pointer", fontSize:"0.9rem", fontFamily:"'Arial Black',sans-serif" }}>✗ RECHAZAR (y perder)</button>
...
function DuelRejectedResultScreen({ players, winnerId, myUserId, onFinalize }: { players: BattlePlayer[]; winnerId: string | null; myUserId: string; onFinalize: () => void }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, padding:24, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <h2 style={{ color: amWinner?"#22c55e":"#ef4444", margin:0, fontSize:"clamp(1.6rem,5vw,2.4rem)", textAlign:"center", zIndex:1 }}>{amWinner?"¡GANASTE!":"PERDISTE"}</h2>
...
function PactScreen({ players, onFinalize }: { players: BattlePlayer[]; onFinalize: () => void }) {
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, gap:24, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"0.85rem", fontFamily:"'Segoe UI',sans-serif", textAlign:"center", maxWidth:290, lineHeight:1.7, zIndex:1, margin:0 }}>Ambos guerreros se respetan. Cada uno se lleva <strong style={{color:"#f59e0b"}}>{PRIZE_PACT.toLocaleString()} pts</strong>.</p>
...
function FinalResultScreen({ players, winnerId, myUserId, onFinalize }: { players: BattlePlayer[]; winnerId: string | null; myUserId: string; onFinalize: () => void }) {
  const sorted=[ ...players].sort((a,b)=>b.score-a.score), winner=players.find(p=>p.userId===winnerId);
  const perfect=winner&&winner.score>=PTS_CORRECTA*MAX_PREGUNTAS, prize=perfect?PRIZE_PERFECT:PRIZE_WIN, amWinner=winnerId===myUserId;
  return (
    <div style={{ minHeight:"100vh", background:"#080810", display:"flex", flexDirection:"column", alignItems:"center", padding:"24px 16px 32px", gap:20, fontFamily:"'Arial Black',Impact,sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      {[...Array(20)].map((_,i) => (<div key={i} style={{ position:"absolute", left:`${Math.random()*100}%`, top:`${Math.random()*60}%`, width:Math.random()*6+2, height:Math.random()*6+2, borderRadius:"50%", background:["#f97316","#fbbf24","#22c55e","#00c8ff","#a855f7"][i%5], opacity:0.6, animation:`confettiFall ${1.5+Math.random()*2}s ease ${Math.random()*2}s infinite` }}/>))}
      <div style={{ fontSize:"0.65rem", letterSpacing:"0.4em", color:"#f97316", zIndex:1 }}>FIN DE LA BATALLA</div>
      {winner && (<><div style={{fontSize:"clamp(2rem,8vw,4rem)"}}>{amWinner?"🏆":"👑"}</div><h2 style={{ color:amWinner?"#fbbf24":"#fff", margin:0, fontSize:"clamp(1.4rem,5vw,2.2rem)", textAlign:"center", zIndex:1, textShadow:amWinner?"0 0 30px rgba(251,191,36,0.6)":"none" }}>{amWinner?"¡GANASTE!":`${winner.username} ganó`}</h2><div style={{ fontSize:"2rem", fontWeight:900, color:"#fbbf24", zIndex:1 }}>{prize.toLocaleString()} pts{perfect&&<span style={{fontSize:"0.8rem",color:"#a855f7",marginLeft:8}}>¡PERFECTO!</span>}</div></>)}
      <div style={{ display:"flex", flexDirection:"column", gap:8, width:"100%", maxWidth:400, zIndex:1 }}>
        {sorted.map((p,i) => { const isWin=p.userId===winnerId; return (
          <div key={p.userId} style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderRadius:12, position:"relative", overflow:"hidden", border:`${isWin?1.5:1}px solid ${isWin?"rgba(251,191,36,0.5)":"rgba(255,255,255,0.06)"}` }}>
            ...
            <div style={{flex:1,zIndex:1}}><div style={{color:isWin?"#fbbf24":"#6b7280",fontFamily:"'Segoe UI',sans-serif",fontWeight:700,fontSize:"0.85rem"}}>{p.username}{p.userId===myUserId&&" (tú)"}</div><div style={{fontSize:"0.65rem",color:isWin?"rgba(251,191,36,0.6)":"rgba(255,255,255,0.15)",fontFamily:"'Segoe UI',sans-serif"}}>{isWin?`Gana ${prize.toLocaleString()} pts`:`Obtuvo ${p.score.toLocaleString()} pts`}</div></div>
...
function Spinner({ size=32 }: { size?: number }) {
  return (<><div style={{ width:size, height:size, borderRadius:"50%", border:`${Math.max(2,Math.floor(size/12))}px solid rgba(249,115,22,0.3)`, borderTopColor:"#f97316", animation:"spinA 0.75s linear infinite", flexShrink:0 }}/><style>{`@keyframes spinA{to{transform:rotate(360deg)}}`}</style></>);
}
...
function AvatarMed({ url, name, size=44, style:extraStyle }: { url?: string|null; name:string; size?:number; style?: React.CSSProperties }) {
  return (
    <div style={{ width:size, height:size, borderRadius:"50%", overflow:"hidden", flexShrink:0, border:"2px solid rgba(255,255,255,0.12)", ...extraStyle }}>
      {url ? <img src={url} alt={name} style={{width:"100%",height:"100%",objectFit:"cover"}}/> : <div style={{ width:"100%",height:"100%", background:"linear-gradient(135deg,#f97316,#e05000)", display:"flex",alignItems:"center",justifyContent:"center", fontSize:size*0.35, fontWeight:900, color:"#fff" }}>{name[0].toUpperCase()}</div>}
    </div>
  );
}
