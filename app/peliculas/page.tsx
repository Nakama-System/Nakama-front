...
> = {
  accion: {
    accent: "#e63946",
    glow: "rgba(230,57,70,.18)",
    bg: "rgba(230,57,70,.07)",
  },
  aventura: {
    accent: "#ff9f43",
    glow: "rgba(255,159,67,.18)",
    bg: "rgba(255,159,67,.07)",
  },
  comedia: {
    accent: "#26de81",
    glow: "rgba(38,222,129,.18)",
    bg: "rgba(38,222,129,.07)",
  },
  drama: {
    accent: "#a29bfe",
    glow: "rgba(162,155,254,.18)",
    bg: "rgba(162,155,254,.07)",
  },
  terror: {
    accent: "#636e72",
    glow: "rgba(99,110,114,.18)",
    bg: "rgba(99,110,114,.07)",
  },
  romance: {
    accent: "#fd79a8",
    glow: "rgba(253,121,168,.18)",
    bg: "rgba(253,121,168,.07)",
  },
  "sci-fi": {
    accent: "#00cec9",
    glow: "rgba(0,206,201,.18)",
    bg: "rgba(0,206,201,.07)",
  },
  animacion: {
    accent: "#fdcb6e",
    glow: "rgba(253,203,110,.18)",
    bg: "rgba(253,203,110,.07)",
  },
  documental: {
    accent: "#b2bec3",
    glow: "rgba(178,190,195,.18)",
    bg: "rgba(178,190,195,.07)",
  },
  diversion: {
    accent: "#55efc4",
    glow: "rgba(85,239,196,.18)",
    bg: "rgba(85,239,196,.07)",
  },
  amor: {
    accent: "#e84393",
    glow: "rgba(232,67,147,.18)",
    bg: "rgba(232,67,147,.07)",
  },
  familia: {
    accent: "#74b9ff",
    glow: "rgba(116,185,255,.18)",
    bg: "rgba(116,185,255,.07)",
  },
  otro: {
    accent: "#dfe6e9",
    glow: "rgba(223,230,233,.18)",
    bg: "rgba(223,230,233,.07)",
  },
};
    ...
    <div className="nk-hero">
      ...
      <div className="nk-hero__content">
        <div className="nk-hero__eyebrow">
          ...
          <span className="nk-hero__pill">
            {total > 0 ? `${total}+ títulos` : "500+ títulos"}
          </span>
          <span className="nk-welcome__role">
              <a
                href="/"
                className="rp-topbar__back"
                title="Ir al inicio"
                aria-label="Inicio"
              >
                <House size={15} />
        ...
        <h1 className="nk-hero__title">
          <span className="nk-hero__title-naka">
            NA<span className="nk-hero__title-ka">KA</span>MA
          </span>
    ...
    <div className="nk-stars">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Star
            size={12}
            fill={n <= Math.round(hover || value) ? "currentColor" : "none"}
          />
        </button>
      ))}
    </div>
...
function ShareModal({
  ...
  return (
    <div className="nk-overlay" onClick={onClose}>
      <div
        className="nk-modal nk-share-modal"
        onClick={(e) => e.stopPropagation()}
      >
        <button type="button" className="nk-modal__close" onClick={onClose}>
          <X size={15} />
        </button>
        ...
        <div className="nk-share-modal__body">
          ...
          <p className="nk-share-modal__desc">
          ...
          {!meta ? (
            ...
            <>
              <div className="nk-share-modal__url">
                ...
                <button
                  type="button"
                  className={`nk-share-modal__copy ${copied ? "done" : ""}`}
                  onClick={copy}
                >
                  {copied ? (
                    <>
          ...
          <button type="button" className="nk-modal__cancel" onClick={onClose}>
            <X size={11} /> Cerrar
          </button>
        </div>
      </div>
...
function MovieCard({
  ...
  const pal = CAT_PALETTE[movie.category] ?? {
    accent: "#e63946",
    glow: "rgba(230,57,70,.18)",
    bg: "rgba(230,57,70,.07)",
  };
  const CatIcon =
    CATEGORIES.find((c) => c.key === movie.category)?.Icon ?? Film;
  const catLbl =
    CATEGORIES.find((c) => c.key === movie.category)?.label ?? movie.category;

  const snapshot = useRef({ rating: movie.rating, votes: movie.votesCount });

  const handleVote = async (n: number) => {
    if (!isAuthenticated) {
      onToast("Inicia sesión para votar", "info");
      return;
    }
    if (voted) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRating(snapshot.current.rating);
        setVotes(snapshot.current.votes);
        setVoted(false);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        a.download = res.filename ?? movie.title;
        a.click();
    ...
    >
      ...
      <div className="nk-card__img" onClick={goToPlayer}>
        {!imgError ? (
          ...
          <div className="nk-card__img-fallback">
            <Film size={32} />
          </div>
        )}
        ...
        {movie.canDownload && (
          <span className="nk-card__dl-badge">
            <Download size={9} /> DL
          </span>
        )}
      ...
      <div className="nk-card__body">
        ...
        <div className="nk-card__rating">
          <StarRating
            ...
            voted={voted}
          />
          <span className="nk-card__score" style={{ color: pal.accent }}>
            {rating.toFixed(1)}
          </span>
          <span className="nk-card__votes">({votes.toLocaleString("es")})</span>
          {voted && (
            <span className="nk-card__voted">
              <Check size={9} /> Votado
            </span>
          )}
          {!isAuthenticated && (
            <span className="nk-card__hint">Inicia sesión para votar</span>
          )}
        </div>
      ...
      <div
        className="nk-card__accent-line"
  ...
  return (
    <div className="nk-pager">
      ...
      <div className="nk-pager__nums">
        ...
        {pages().map((p, i) =>
          p === "…" ? (
            <span key={`el-${i}`} className="nk-pager__ellipsis">
              …
            </span>
          ) : (
            <button
...
export default function PeliculasPage() {
  ...
  const handleAgeRating = (age: AgeRating) => {
    setAgeRating(age);
    setPage(1);
  };
    ...
    <main className="nk-page">
      ...
      {isAuthenticated && user && (
        <div className="nk-welcome">
          {user.avatarUrl && (
            <img src={user.avatarUrl} alt="" className="nk-welcome__av" />
          )}
          <div className="nk-welcome__text">
            ...
            {user.subscription?.active && (
              <span className="nk-welcome__plan">
      ...
      <section className="nk-catalog" ref={catalogRef}>
        <div className="nk-catalog__head">
          <div className="nk-catalog__heading">
            ...
            <span className="nk-catalog__count">
              {loading ? "…" : total} películas
            </span>
          </div>
        ...
        <div className={`nk-grid ${loading ? "nk-grid--busy" : ""}`}>
          {error ? (
            ...
            <div className="nk-grid__spinner">
              <div className="nk-spinner" />
            </div>
          ) : movies.length === 0 ? (
            <div className="nk-grid__empty">
      ...
      {/* Modales */}
      {shareMovie && (
        <ShareModal movie={shareMovie} onClose={() => setShareMovie(null)} />
      )}
