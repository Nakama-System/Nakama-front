  ...
  return (
    <div className="agenda-root">
      ...
      <div className="agenda-search">
        ...
        <input
          className="agenda-search__input"
          ...
          {sources.map((s) => (
            <button
              ...
              style={
                filter === s && s !== "all"
                  ? {
                      borderColor: SOURCE_COLORS[s as UserSuggestion["source"]],
                      color: SOURCE_COLORS[s as UserSuggestion["source"]],
                    }
                  : {}
              }
            >
      ...
      <div className="agenda-grid-wrap">
        {loadingContacts ? (
          ...
          <div className="agenda-empty">
            ...
            <p>
              {search || filter !== "all"
                ? "Intentá con otro filtro o nombre"
                : "Agregá amigos para chatear fácilmente"}
            </p>
            {!search && filter === "all" && (
              <button className="agenda-empty__cta" onClick={onAdd}>
                <UserPlus size={14} /> Agregar nakama
              </button>
            )}
          </div>
        ) : (
          <div className="agenda-grid">
            {filtered.map((c, idx) => {
              ...
              // Detecta video (mp4/webm/mov) o gif — ambos se animan
              const profileVideoUrl = (c as any).profileVideo?.url;
              const avatarIsAnimated = isAnimatedUrl(c.avatarUrl);
              const animatedSrc =
                profileVideoUrl || (avatarIsAnimated ? c.avatarUrl : undefined);
              const staticSrc =
                !profileVideoUrl && !avatarIsAnimated ? c.avatarUrl : undefined;
              ...
              return (
                <div
                  ...
                  className="agenda-card"
                  style={
                    { "--ag-card-accent": src.color } as React.CSSProperties
                  }
                >
                  {confirmId === c._id && (
                    <div
                      className="agenda-card__confirm"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <p>
                        ¿Eliminar a <strong>@{c.username}</strong>?
                      </p>
                      <div className="agenda-card__confirm-btns">
                        <button
                          className="agenda-card__confirm-yes"
                          onClick={() => {
                            onRemove(c._id);
                            setConfirmId(null);
                          }}
                        ...
                        <button
                          className="agenda-card__confirm-no"
                  ...
                  <div className="agenda-card__avatar-wrap">
                    ...
                    <div
                      className="agenda-card__source-badge"
                  ...
                  {c.mutualCount ? (
                    <span
                      className="agenda-card__mutual"
                      style={{ color: src.color }}
                    >
                      {c.mutualCount} en común
                    </span>
                  ) : null}
                  <div className="agenda-card__actions">
                    <button
                      className="agenda-card__action agenda-card__action--chat"
                      onClick={() => onChat(c)}
                      title="Chatear"
                    >
                    ...
                    <button
                      className="agenda-card__action agenda-card__action--remove"
...
export function AddContactModal({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch(
        `${API}/chats/search-users?q=${encodeURIComponent(query)}&source=all`,
        ...
        .then((d) =>
          setResults(
            Array.isArray(d)
              ? d.filter((u: UserSuggestion) => u._id !== currentUserId)
              : [],
          ),
        )
        .catch(() => {})
    ...
    <div className="agenda-modal-overlay" onClick={onClose}>
      <div className="agenda-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="agenda-modal__search-wrap">
          <Search
            size={14}
            style={{ color: "rgba(240,240,248,0.4)", flexShrink: 0 }}
          />
          <input
            autoFocus
        ...
        <div className="agenda-modal__results">
          ...
          {!loading && query.length >= 2 && results.length === 0 && (
            <div className="agenda-modal__no-results">
          ...
          {!loading &&
            results.map((u) => {
              ...
              const uIsAnimated = isAnimatedUrl(u.avatarUrl);
              return (
                <div
                  key={u._id}
                  className={`agenda-modal__result ${isIn ? "agenda-modal__result--added" : ""}`}
                >
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <UserAvatar
                      videoSrc={uIsAnimated ? u.avatarUrl : undefined}
                      src={!uIsAnimated ? u.avatarUrl : undefined}
                      alt={u.username}
                      size={36}
                    />
                    <div
                  ...
                  <div className="agenda-modal__result-info">
                    <span className="agenda-modal__result-name">
                      @{u.username}
                    </span>
                    <span
                      className="agenda-modal__result-sub"
                      style={{ color: src.color }}
                    >
                      {src.label}
                    </span>
                  ...
                  {isIn ? (
                    <span className="agenda-modal__added-label">
                      <Check size={12} /> Agregado
                    </span>
                  ) : (
                    <button
                      className="agenda-modal__add-btn"
                      disabled={inProgress}
                      onClick={() => handleAdd(u)}
                    >
                      {inProgress ? (
...
// ══════════════════════════════════════════════════════════
// ConvItem — avatar animado si avatarUrl es video o gif
// ══════════════════════════════════════════════════════════
export function ConvItem({
  ...
  const videoSrc =
    conv.profileVideo?.url ||
    (isAnimatedUrl(conv.avatarUrl) ? conv.avatarUrl : undefined);
  const imgSrc = !videoSrc ? conv.avatarUrl : undefined;
    ...
    >
      <div className="chat-conv-item__avatar">
        ...
        {conv.type === "private" && conv.online && (
          <span className="chat-conv-item__online" />
        )}
        {conv.type === "group" && (
          <span className="chat-conv-item__type-badge">
            <Users size={8} />
          </span>
        )}
        {conv.type === "community" && (
        ...
        {conv.isBlocked && (
          <span className="chat-conv-item__blocked-badge">
            <Ban size={8} />
          </span>
        )}
      </div>
      <div className="chat-conv-item__body">
        <div className="chat-conv-item__top">
          <span className="chat-conv-item__name">
            {conv.name}
            {conv.muted && (
              <BellOff size={11} className="chat-conv-item__muted" />
            )}
            {conv.isBlocked && (
              <Ban size={11} className="chat-conv-item__blocked-icon" />
            )}
          </span>
...
export function NewChatModal({
  ...
  const [searchSource, setSearchSource] = useState<
    "all" | "contactos" | "tiktok" | "instagram" | "facebook"
  >("all");
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("nakama_token");
    fetch(
      `${API}/chats/search-users?q=${encodeURIComponent(searchQ)}&source=${searchSource}`,
      { headers: { Authorization: `Bearer ${token}` } },
    )
      .then((r) => r.json())
      .then((d) =>
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch(`${API}/chats`, {
        ...
        body: JSON.stringify({
          type: type === "chat" ? "private" : "group",
          name: type === "chat" ? selected[0]?.username : groupName.trim(),
      ...
      onCreated({
        ...conv,
        name:
          type === "chat" ? (selected[0]?.username ?? conv.name) : conv.name,
        avatarUrl:
          type === "chat"
            ? (selected[0]?.avatarUrl ?? conv.avatarUrl)
            : conv.avatarUrl,
        unread: 0,
        otherId: type === "chat" ? selected[0]?._id : undefined,
      });
    } catch (err) {
    ...
    <div className="chat-modal-overlay" onClick={onClose}>
      <div className="chat-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="chat-modal__sources">
          {(
            ["all", "contactos", "tiktok", "instagram", "facebook"] as const
          ).map((s) => (
            <button
              key={s}
              className={`chat-modal__source-btn ${searchSource === s ? "chat-modal__source-btn--active" : ""}`}
        ...
        <div className="chat-modal__results">
            ...
            return (
              ...
              >
                ...
                <div className="chat-modal__result-info">
                  ...
                  <span
                    className={`chat-modal__result-source chat-modal__result-source--${u.source}`}
                  >
                    {sourceConfig(u.source).label}
                  </span>
                </div>
          ...
          {searchQ.length >= 2 && displayList.length === 0 && (
            <div className="chat-modal__no-results">
              No se encontraron usuarios
            </div>
          )}
        </div>
        <button
          ...
          disabled={
            creating ||
            (type === "chat" ? selected.length === 0 : !groupName.trim())
          }
...
export function ThemeModal({
  ...
  return (
    <div className="chat-modal-overlay" onClick={onClose}>
      <div
        className="chat-modal chat-modal--themes"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="chat-modal__header">
          <Palette size={20} />
        ...
        <div className="chat-theme-grid">
          {themes.map((t) => (
            ...
            >
              ...
              <span>{t.label}</span>
              {activeTheme === t.id && (
                <Check size={14} className="chat-theme-card__check" />
              )}
            </button>
          ))}
        </div>
